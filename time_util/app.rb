...
parser.make_flight_objects
	...
	def hours_to_minutes
		...
		hours = format_flights.select.each_with_index{|num, i| i.even?}#values_at(0,2,4,6,8)
		hours.collect{|hour| hour * 60}
	end

	def add_extra_minutes
		#need to convert this to odds
		minutes = format_flights.select.each_with_index{|num, i| i.odd?}#values_at(1,3,5,7,9)
